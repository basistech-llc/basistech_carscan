...
def get_bucket_from_samconfig(config_path="samconfig.toml"):
    ...
    try:
        ...
        # Access the parameter_overrides string
        params_str = config['default']['deploy']['parameters'].get('parameter_overrides', "")
        
        # Use regex to find ImageBucketName="value"
...
def enable_eventbridge(bucket_name, region):
    """Enables EventBridge notifications for the specified S3 bucket."""
    s3 = boto3.client('s3', region_name=region)
    
    print(f"--- Enabling EventBridge for bucket: {bucket_name} ---")
...
if __name__ == "__main__":
    ...
    enable_eventbridge(target_bucket, args.region)
